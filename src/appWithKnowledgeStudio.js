...
export function createRequestHandler(config, dependencies = {}) {
  ...
  return async function knowledgeStudioAwareHandler(req, res) {
    ...
    if (!requestUrl.pathname.startsWith('/api/admin/knowledge-studio')) {
      return base(req, res);
    }
    ...
    try {
      ...
      if (section === 'overview' && req.method === 'GET') {
        return sendJson(res, 200, { ok: true, overview: await studio.overview() });
      }

      if (section === 'predicates' && req.method === 'GET') {
        return sendJson(res, 200, { ok: true, predicates: await studio.predicates() });
      }
      ...
      if (section === 'entities' && !id && req.method === 'GET') {
        const result = await studio.listEntities({
          ...
          limit: positiveInt(requestUrl.searchParams.get('limit'), 50),
          offset: positiveInt(requestUrl.searchParams.get('offset'), 0, 100000)
        });
        return sendJson(res, 200, { ok: true, ...result });
      }

      if (section === 'entities' && id && req.method === 'GET') {
        const entity = await studio.entityDetails(id);
        return entity
          ? sendJson(res, 200, { ok: true, entity })
          : sendJson(res, 404, { error: 'ENTITY_NOT_FOUND' });
      }

      if (section === 'facts' && !id && req.method === 'GET') {
        const result = await studio.listFacts({
          query: requestUrl.searchParams.get('query') ?? '',
          predicate: requestUrl.searchParams.get('predicate') ?? '',
          status: requestUrl.searchParams.get('status') ?? '',
          needsReview: requestUrl.searchParams.get('needsReview') === 'true',
          limit: positiveInt(requestUrl.searchParams.get('limit'), 50),
          offset: positiveInt(requestUrl.searchParams.get('offset'), 0, 100000)
        });
        return sendJson(res, 200, { ok: true, ...result });
      ...
      if (section === 'facts' && id && action === 'review' && req.method === 'POST') {
        ...
        const fact = await studio.reviewFact({
          factId: id,
          action: payload.action,
          comment: payload.comment ?? '',
          actor: admin.email ?? admin.name ?? 'admin'
        });
        return fact
          ? sendJson(res, 200, { ok: true, fact })
          : sendJson(res, 404, { error: 'FACT_NOT_FOUND' });
      }

      if (section === 'facts' && id && req.method === 'PUT') {
        const payload = await readJson(req);
        const fact = await studio.updateFact({
          factId: id,
          predicate: payload.predicate,
          objectEntityId: payload.objectEntityId ?? null,
      ...
      const status = error?.code === 'POSTGRES_REQUIRED' ? 503
        : ['INVALID_REVIEW_ACTION', 'UNKNOWN_PREDICATE', 'FACT_VALUE_REQUIRED', 'INVALID_JSON'].includes(error?.code) ? 422
        : error?.code === 'BODY_TOO_LARGE' ? 413
        : 500;
      console.error('[knowledge-studio]', error?.code ?? 'UNKNOWN', error?.message ?? error);
      return sendJson(res, status, { error: error?.code ?? 'KNOWLEDGE_STUDIO_ERROR', message: error?.message ?? 'Knowledge Studio error' });
    }
  };
}
