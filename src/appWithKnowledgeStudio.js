...
export function createRequestHandler(config, dependencies = {}) {
  ...
  return async function knowledgeStudioAwareHandler(req,res){
    ...
    try{
      ...
      if(section==='entities'&&id&&action==='history'&&req.method==='GET')return sendJson(res,200,{ok:true,items:await entityEditor.history(id,positiveInt(requestUrl.searchParams.get('limit'),100,300))});
      if(section==='entities'&&id&&action==='merge-preview'&&req.method==='POST'){
        const payload=await readJson(req); const preview=await entityMerge.preview({sourceEntityId:id,targetEntityId:payload.targetEntityId});
      ...
      if(section==='entities'&&id&&action==='merge'&&req.method==='POST'){
        const payload=await readJson(req); const result=await entityMerge.merge({sourceEntityId:id,targetEntityId:payload.targetEntityId,comment:payload.comment??'',actor:actor(admin)});
        return sendJson(res,200,{ok:true,result});
      }
      if(section==='entities'&&id&&action==='aliases'&&!nestedId&&req.method==='POST'){
        const payload=await readJson(req); const alias=await entityEditor.addAlias({entityId:id,alias:payload.alias,language:payload.language??null,comment:payload.comment??'',actor:actor(admin)});
      ...
      if(section==='entities'&&id&&action==='aliases'&&nestedId&&req.method==='DELETE'){
        const payload=await readJson(req); const alias=await entityEditor.deleteAlias({entityId:id,aliasId:nestedId,comment:payload.comment??'',actor:actor(admin)});
        return alias?sendJson(res,200,{ok:true,alias}):sendJson(res,404,{error:'ALIAS_NOT_FOUND'});
      }
      if(section==='entities'&&id&&req.method==='PUT'){
        const payload=await readJson(req); const entity=await entityEditor.updateEntity({entityId:id,canonicalName:payload.canonicalName,entityType:payload.entityType,shortDescription:payload.shortDescription??null,status:payload.status,comment:payload.comment??'',actor:actor(admin)});
      ...
      if(section==='facts'&&!id&&req.method==='GET'){
        const result=await studio.listFacts({query:requestUrl.searchParams.get('query')??'',predicate:requestUrl.searchParams.get('predicate')??'',status:requestUrl.searchParams.get('status')??'',needsReview:requestUrl.searchParams.get('needsReview')==='true',sort:requestUrl.searchParams.get('sort')??'updated',order:requestUrl.searchParams.get('order')??'desc',limit:positiveInt(requestUrl.searchParams.get('limit'),50),offset:positiveInt(requestUrl.searchParams.get('offset'),0,100000)});
        return sendJson(res,200,{ok:true,...result});
      }
