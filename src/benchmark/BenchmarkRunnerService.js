...
export class BenchmarkRunnerService{
  ...
  async run({datasetId,onProgress=()=>{}}){
    ...
    for(let i=0;i<items.length;i++){
      ...
      const automatedPass=!error&&!refused&&answerPresent&&policy.passed&&(item.polarity==='negative'?refused:factResults.every(x=>x.passed));
      ...
      else if(!answerPresent)diagnosis='EMPTY_OR_TOO_SHORT';
      else if(!policy.passed)diagnosis=policy.diagnosis;
